...
\begin{center}
\textbf{fundamental diagram of Netwon's method}
\end{center}
...
> $\color{gray}// Initial guess: 1$
> 1
1
> $\color{gray}// Step 1a. Check if the current guess is close enough.$
> 1 - 2/1
0.5
> $\color{gray}// 0.5 is not close enough. Keep going.$
> $\color{gray}// Step 2. Update the current guess.$
> (1 + 2/1)/2
1.5
> $\color{gray}// The new guess is 1.5.$
> $\color{gray}// Step 3, go back to Step 1.$
> $\color{gray}// Step 1a. Check if the current guess is close enough.$
> 1.5 - 2/1.5
0.16666666666666674
...
> $\color{gray}// 0.004901960784313486 is 2 decimal places correct. But we want three. Keep going.$
> $\color{gray}// Step 2. Update the current guess.$
...
> $\color{gray}// Step 3, go to Step 1 again.$
> $\color{gray}// Check: 0.0000042477996395895445 tells us we are correct to 5 decimal places. Stop! Our current guess is good enough. Return the current guess {\color{cyan}1.4142156862745097}. That value is our output.$
\end{lstlisting}
...
\begin{question}
  Use Newton's method to calculate $\sqrt{4}$ to three decimal places. How many iterations of the loop did you execute in your calculation?
\end{question}

\begin{question}
  Use Newton's method to calculate $\sqrt{5}$ to three decimal places. How many iterations of the loop did you execute in your calculation?
\end{question}
